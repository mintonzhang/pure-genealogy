...
- **自包含架构**: 使用 **SQLite** (better-sqlite3) 作为数据库，零依赖部署，数据文件即数据库。
...
- **数据库**: [SQLite](https://www.sqlite.org/) + [better-sqlite3](https://github.com/WiseLibs/better-sqlite3)（同步访问，自动迁移）
- **认证**: 自建 JWT 认证 (jose + bcryptjs)
...
首次启动后，访问 `/auth/sign-up` 注册管理员账户，之后即可录入家族成员数据。
